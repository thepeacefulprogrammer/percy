...
from ventures_agent_framework import agent
...
async def non_streaming_polling() -> None:
    ...
    print(
        f"Initial status: continuation_token={'set' if response.continuation_token else 'None'}"
    )
    ...
    while response.continuation_token is not None:
        ...
        print(
            f"  Poll {poll_count}: continuation_token={'set' if response.continuation_token else 'None'}"
        )
