...
SLASH_COMMANDS: dict[str, str] = {
    ...
    "todo-add": "Add a TODO: /todo-add <title> [project] [priority]",
    "todo-start": "Mark TODO in progress: /todo-start <id>",
    "todo-done": "Mark TODO done: /todo-done <id>",
    "todo-note": "Append note: /todo-note <id> <note>",
    "todo-show": "Show TODO details: /todo-show <id>",
    "todo-next": "Show next TODO",
}
...
async def graph_query(
    cypher: str, params: dict | None = None, database: str | None = None
) -> str:
    try:
...
async def need_further_action(prompt: str) -> QuickResponse:
    response = await agent(
        name="Percy",
        prompt=prompt,
        instructions="You are Percy. An AI assistant with tools. You are action oriented. You are participating in a multi-step workflow where you are the first step after receiving the user's prompt. You decide if you can just action the request or if it is a multi-step process. (i.e. a task though would require multiple tool calls.\n\nReturn a one-two sentence message to the user that either: a) fulfills the request b) acknowledges that you will action the request.\n\nIn the case that multi-step actions need to occur, just acknowledge what the user is wanting and that you are going to start working on it, and set need_further_action to True. Do not take the further action yourself..\n\nBut, if it is just a simple request, then just action it by calling the tools and provide your response.\n\n#IMPORTANT\nThis should seem like one continous conversation for the user - so respond as if you are responding as part of the workflow. DO NOT discuss anything your decision making or about whether this required multiple actions or does not. Just respond naturally directly to the prompt. IF THIS IS SOMETHING THAT CAN BE RESOLVED IN ONE OR TWO TOOL CALLS, MAKE THE TOOL calls and complete the action.",
        tools=percy_tools,
        response_format=QuickResponse,
        middleware=percy_middleware,
    )

    if isinstance(response, QuickResponse):
        return response
    else:
        return QuickResponse(message=f"{response}", further_action_required=True)


async def send_to_agent(prompt: str) -> str:
    quick_response: QuickResponse = await need_further_action(prompt)

    if quick_response.further_action_required == False:
        return quick_response.message

    logger.info(quick_response.message)

    handover: HandOver = context.handover
    handover_prompt = await handover.get_handover_prompt(prompt)

    handover_prompt += f"\nQuick call to the model with this request determined further action was required and logged this update to the user: {quick_response.message}. Carry on to action the request."
    ...
    while True:
        ...
        if isinstance(response, HandOverAgentUpdate):
            ...
            logger.info(f"Recieved a response that was unexpected: {response}")
    ...
    async def _handle_slash_command(command: str, args: list[str]) -> bool:
        ...
        def _render_todos(todos: list[dict]) -> str:
            ...
            for todo in todos:
                ...
                lines.append(
                    f"{todo.get('id')} [{todo.get('priority', 'P2')}] {todo.get('title')}{project_str} — {status}"
                )
            return "\n".join(lines)
        ...
        if command == "todo":
            store = get_todo_store(config.project.output_dir)
            todos = store.list()
            active = [t for t in todos if t.get("status") in {"open", "in_progress"}]
            console.print(Panel(_render_todos(active), title="Active TODOs"))
            return True

        if command == "todo-add":
            if not args:
                console.print(
                    "[system]Usage: /todo-add <title> [project] [priority][/system]"
                )
                return True
            title = args[0]
            project = args[1] if len(args) >= 2 else None
            priority = args[2] if len(args) >= 3 else "P2"
            store = get_todo_store(config.project.output_dir)
            todo = store.add(title=title, project=project, priority=priority)
            console.print(Panel(_render_todos([todo]), title="TODO Added"))
            return True

        if command == "todo-start":
            if not args:
                console.print("[system]Usage: /todo-start <id>[/system]")
                return True
            store = get_todo_store(config.project.output_dir)
            todo = store.set_status(args[0], "in_progress")
            console.print(Panel(_render_todos([todo]), title="TODO Updated"))
            return True

        if command == "todo-done":
            if not args:
                console.print("[system]Usage: /todo-done <id>[/system]")
                return True
            store = get_todo_store(config.project.output_dir)
            todo = store.set_status(args[0], "done")
            console.print(Panel(_render_todos([todo]), title="TODO Updated"))
            return True

        if command == "todo-note":
            if len(args) < 2:
                console.print("[system]Usage: /todo-note <id> <note>[/system]")
                return True
            todo_id = args[0]
            note = " ".join(args[1:])
            store = get_todo_store(config.project.output_dir)
            todo = store.append_note(todo_id, note)
            console.print(Panel(_render_todos([todo]), title="TODO Updated"))
            return True

        if command == "todo-show":
            if not args:
                console.print("[system]Usage: /todo-show <id>[/system]")
                return True
            store = get_todo_store(config.project.output_dir)
            todo = store.get(args[0])
            payload = json.dumps(todo, ensure_ascii=False, indent=2)
        ...
        if command == "todo-next":
            ...
            console.print(
                Panel(_render_todos([todo] if todo else []), title="Next TODO")
            )
            return True
